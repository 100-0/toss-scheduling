...
export default function CompleteScreen() {
  ...
  const impossibleMembers = MEMBERS.filter(
    (m) => selectedSlot.statuses[m.id] === "impossible",
  );
    ...
    <div className="relative flex flex-col h-full bg-gray-03">
      ...
      <div className="flex flex-col gap-4 items-center px-5 w-full">
        <div className="flex flex-col gap-4 items-center pb-4 pt-8 w-full">
          ...
          <div className="text-[19px] font-bold text-gray-90">
            일정이 확정됐어요
          </div>
        </div>
        <div
        ...
        >
          <div
            aria-hidden
            className="absolute inset-0 pointer-events-none rounded-[inherit] shadow-[inset_0_0_30px_0_rgba(255,255,255,0.4)]"
          />
          <div className="flex flex-col gap-4 items-start w-full">
            <div className="text-[17px] font-semibold text-gray-10">
              {MEETING_TITLE}
            </div>
          ...
          <div className="flex flex-col gap-3 items-start w-full">
              ...
              <div className="flex items-start">
                {attendingMembers.map((m, i) => (
                  <div
                    key={m.id}
                    style={{
                      marginRight: i === attendingMembers.length - 1 ? 0 : -12,
                    }}
                  >
                    <Avatar
                      id={m.id}
            ...
            {impossibleMembers.length > 0 && (
              <div className="flex flex-col gap-1 items-start w-full">
                <div className="text-[13px] font-medium text-gray-10">
                  미참석자
                </div>
                <div className="flex items-start">
                  {impossibleMembers.map((m, i) => (
                    <div
                      key={m.id}
                      style={{
                        marginRight:
                          i === impossibleMembers.length - 1 ? 0 : -12,
                      }}
                    >
                      <Avatar
                        id={m.id}
        ...
        <div className="text-[15px] text-gray-95">
          확정된 회의 일정을 전원에게 보냈어요.
        </div>
      </div>
