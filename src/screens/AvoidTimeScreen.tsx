...
export default function AvoidTimeScreen() {
  ...
  const weeks = groupByWeek(
    weekdaysInRange(meetingRange.start, meetingRange.end),
  );
  ...
  return (
    <div className="relative flex flex-col h-full bg-gradient-to-b from-gray-05 to-[#e6e7ea]">
      ...
      <div className="flex flex-col items-start w-full shrink-0">
        <Header
          title=""
          onBack={isHost ? () => setScreen("home") : undefined}
        />
        ...
        <div className="flex flex-col gap-2 items-start px-5 w-full">
          <div className="text-[24px] font-bold text-gray-95 leading-[1.35]">
            참석이 어려운 시간이 있나요?
          </div>
          <div className="text-[15px] text-gray-70 leading-[1.55] w-full">
            {name}님의 캘린더에서 일정을 불러와 미리 입력해두었어요. 터치하거나
            드래그해서 시간을 제외해주세요.
          </div>
        </div>
      </div>
      {weeks.length > 1 && (
        <div className="px-4 pt-5 pb-3 w-full shrink-0">
          <WeekTabs
            weeks={weeks}
