...
const DEAD_STYLE: CSSProperties = {
  ...
  backgroundImage:
    "repeating-linear-gradient(45deg, transparent 0 4px, var(--color-gray-20) 4px 5px)",
};
...
export default function ExploreTimesScreen() {
  ...
  const cellMap = useMemo(
    () => new Map(cells.map((c) => [cellKey(c.date, c.hour), c])),
    [cells],
  );

  const [selected, setSelected] = useState<ExploreCell | null>(null);

  const yielders = selected?.slot
    ? MEMBERS.filter((m) => selected.slot!.statuses[m.id] === "yield")
    : [];
  const impossible = selected?.slot
    ? MEMBERS.filter((m) => selected.slot!.statuses[m.id] === "impossible")
    : [];
  const possible = selected?.slot
    ? MEMBERS.filter((m) => selected.slot!.statuses[m.id] === "possible")
    : [];
  ...
  const statusLabel =
    statusKind === "clear"
      ? "전원 참석 가능"
      : statusKind === "yield"
        ? `양보 ${yielders.length}명 필요`
        : `불가 ${impossible.length}명`;
  ...
  return (
    <div
      className="relative flex flex-col h-full bg-gray-03"
      onClick={() => setSelected(null)}
    >
      <StatusBar />
      <Header title="전체 시간 탐색" onBack={() => setScreen("suggestTime")} />

      <div className="px-5 pb-3 w-full shrink-0 text-[15px] text-gray-60 leading-[1.5]">
        참석 가능 인원이 많을수록 진하게 표시돼요
      </div>

      {weeks.length > 1 && (
        <div className="px-4 pb-3 pt-4 w-full shrink-0">
          <WeekTabs
            weeks={weeks}
            activeIndex={weekIdx}
      ...
      <div className="flex items-center gap-3 px-5 pb-3 w-full shrink-0 text-[12px] text-gray-60">
                  ...
                  {columns.map((date) => (
                    ...
                    >
                      {HOURS.map((h) => {
                        ...
                        if (!cell)
                          return <div key={h} style={{ height: CELL - GAP }} />;
                        const isSelected =
                          !!selected &&
                          selected.date === date &&
                          selected.hour === h;
                          ...
                          return (
                            <button
                              ...
                              className={`w-full rounded-[2px] cursor-pointer ${
                                isSelected
                                  ? "ring-2 ring-inset ring-gray-60"
                                  : ""
                              }`}
                              aria-label="필수 인원 불가"
                            />
                        ...
                        return (
                          <button
                            ...
                            className={`w-full rounded-[2px] cursor-pointer ${DENSITY_BG[level]} ${
                              cell.rank
                                ? "outline outline-1 outline-offset-[-1px] outline-gray-80"
                                : ""
      ...
      <div
        className="shrink-0 z-[1] rounded-t-[24px] bg-gray-00 px-5 pt-5 pb-6 shadow-[0_-6px_24px_0_rgba(0,0,0,0.10)]"
        ...
        {!selected ? (
          ...
          <>
            ...
            <div className="bg-gray-20 rounded-lg px-4 py-3 mb-4">
              <p className="text-[13px] text-gray-95 leading-[1.5]">
                <b>{selected.deadByName}</b>(필수)님이 참석할 수 없는
                시간이에요.
                <br />이 시간은 추천에서 제외했어요.
              </p>
            </div>
            <button
              disabled
              className="w-full h-[52px] rounded-full flex items-center justify-center text-[17px] font-semibold bg-gray-30 text-gray-00 cursor-not-allowed"
            >
              이 시간으로 확정하기
            </button>
          </>
          ...
          <>
            <div className="flex items-start justify-between gap-2 mb-3">
              ...
              <span
                className={`rounded-md px-2 py-1 text-[12px] font-semibold shrink-0 ${STATUS_TAG[statusKind]}`}
              >
                {statusLabel}
              </span>
            </div>
